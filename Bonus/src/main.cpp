...
int main()
{
  ...
  h.onMessage([&ukf,&target_x,&target_y,&isfirst](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    // std::cout<<ukf.x_[3]<<"    "
    //   <<ukf.x_[0]/(ukf.x_[1]*ukf.x_[1]+ukf.x_[0]*ukf.x_[0])+<<std::endl;

    double mid_x=0.0;
    double mid_y=0.0;
    double heading_to_target=0.0;
    double heading_difference=0.0;
    double target_xx=0.0;
    double target_yy=0.0;
    double distance_difference = sqrt((target_y - hunter_y)*(target_y - hunter_y) + (target_x - hunter_x)*(target_x - hunter_x));
    	  if(distance_difference>10 && isfirst==true)
        {
          mid_x = (target_x+hunter_x)/2;
          mid_y = (target_y+hunter_y)/2;
          heading_to_target = atan2(mid_y-hunter_y, mid_x-hunter_x);
        ...
        else
        {
          ...
          {
            t+=0.05;
            target_xx = target_x+ukf.x_[2]/ukf.x_[4]*(sin(ukf.x_[3]+ukf.x_[4]*t)-sin(ukf.x_[3]));
            target_yy = target_y+ukf.x_[2]/ukf.x_[4]*(-cos(ukf.x_[3]+ukf.x_[4]*t)+cos(ukf.x_[3]));
            double dis = (hunter_x-target_xx)*(hunter_x-target_xx)
                +(hunter_y-target_yy)*(hunter_y-target_yy);
            if(dis-ukf.x_[2]*t*ukf.x_[2]*t<=0.1)
              break;
          }
          heading_to_target = atan2(target_yy-hunter_y, target_xx-hunter_x);
          while (heading_to_target > M_PI) heading_to_target-=2.*M_PI; 
          while (heading_to_target <-M_PI) heading_to_target+=2.*M_PI;
          heading_difference = heading_to_target - hunter_heading;
          while (heading_difference > M_PI) heading_difference-=2.*M_PI; 
